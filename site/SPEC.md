> **注記（2026-09-13 Phase 2）**: 本書は v1 実装を委譲した際の仕様書（履歴）。「Forbidden」の編集禁止は委譲先への当時の制約であり、リポジトリの恒久契約ではない。v2（10 分野・動画/広告/SNS の自主制作サンプル追加）以降の構成は `site/README.md` と `site/content.json` を正とする。
